...
export default function NavBar() {
    ...
    // Function to format the roles array for display
    const formatRoles = (roles: string | any[]) => {
    ...
    return (
        <>
            {/* Development indicators - can be hidden in production */}
            <div className="bg-base-200 py-1 px-2 text-xs">
                <div className="max-w-screen-lg mx-auto flex flex-wrap gap-3">
                    <span className="badge badge-primary">
                        Status: {isLoggedIn ? 'Logged In' : 'Logged Out'}
                    </span>
                    {isLoggedIn && roles && roles.length > 0 && (
                        ...
                        {isLoggedIn && (
                            <div className="dropdown dropdown-end">
                                <div className="indicator">
                                    {/* Role badges will appear on avatar */}
                                    {Array.isArray(roles) && roles.length > 0 && (
                                        <span className="indicator-item indicator-center indicator-bottom badge badge-sm font-semibold badge-secondary">
                                            {roles.length === 1
                                ...
                                <ul tabIndex={0} className="menu menu-sm dropdown-content mt-3 z-[3] p-2 shadow bg-base-100 rounded-box w-52">
                                    {/* Show all roles in dropdown if multiple */}
